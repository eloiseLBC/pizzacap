...
const PizzaComponent: React.FC<PizzaComponentProps> = ({ name, image_url, ingredients, price }) => {
  ...
  return (
    <div className="w-72 rounded-3xl border border-green-500 p-4 flex flex-col items-center shadow-md">
      <h2 className="text-xl font-bold text-green-700 mb-2">{name}</h2>
      <img src={image_url} alt={name} className="rounded-xl w-full h-36 object-cover mb-3" />
